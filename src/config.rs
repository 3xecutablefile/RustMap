...
impl Config {
    ...
    pub fn from_args(args: &[String]) -> Result<Self> {
        ...
        let scan_timeout = Self::parse_timeout_arg(args, "--scan-timeout", constants::DEFAULT_SCAN_TIMEOUT_MS)?;
        let exploit_timeout = Self::parse_timeout_arg(args, "--exploit-timeout", constants::DEFAULT_EXPLOIT_TIMEOUT_SECS * 1000)?;
        ...
        let shutdown_timeout = Self::parse_timeout_arg(args, "--shutdown-timeout", env_config.shutdown_timeout.as_millis() as u64)?;
        let enable_rate_limiting = !args.contains(&"--no-rate-limit".to_string()) && env_config.enable_rate_limiting;
    ...
    fn parse_thread_arg(args: &[String], default: usize) -> Result<usize> {
        ...
        Ok(if default == 0 { num_cpus::get() } else { default })
    }
