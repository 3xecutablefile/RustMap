...
/// Search strategy for exploit database queries
...
impl ExploitSearcher {
    ...
    pub async fn search_exploits(
        ...
        let timeout = timeout.unwrap_or(Duration::from_secs(constants::DEFAULT_EXPLOIT_TIMEOUT_SECS));
        ...
        if let Ok(exploits) = self.search_with_strategy(&validated_query, SearchStrategy::Exact, timeout).await {
        ...
        if all_exploits.is_empty() {
            if let Ok(exploits) = self.search_with_strategy(&validated_query, SearchStrategy::Fuzzy, timeout).await {
                for exploit in exploits {
                    if seen_titles.insert(exploit.title.clone()) {
                        all_exploits.push(exploit);
                    }
        ...
        if all_exploits.is_empty() && validated_query.contains(' ') {
            let simple_query = validated_query.split_whitespace().next().unwrap_or(&validated_query);
            if let Ok(exploits) = self.search_with_strategy(simple_query, SearchStrategy::ServiceOnly, timeout).await {
                for exploit in exploits {
                    if seen_titles.insert(exploit.title.clone()) {
                        all_exploits.push(exploit);
                    }
    ...
    pub async fn search_with_strategy(
        ...
        let args = self.build_search_args(query, &strategy);
    ...
    fn build_search_args(&self, query: &str, strategy: &SearchStrategy) -> Vec<String> {
        let mut args = vec!["searchsploit".to_string()];
        
        match strategy {
            SearchStrategy::Exact => {
                args.push("-e".to_string()); // Exact match
            }
            SearchStrategy::Fuzzy => {
                // No additional flags for fuzzy search
            }
            SearchStrategy::ServiceOnly => {
                // No additional flags, just use simpler query
            }
        }
    ...
    fn extract_cvss(&self, text: &str) -> Option<f32> {
        ...
        if text_lower.contains("pre-authentication rce") || text_lower.contains("unauthenticated rce") {
        ...
        } else if text_lower.contains("directory traversal") || text_lower.contains("path traversal") {
        ...
        } else if text_lower.contains("xss") || text_lower.contains("cross site") || 
    ...
    fn sort_exploits(&self, exploits: &mut [Exploit]) {
        exploits.sort_by(|a, b| {
            match (a.cvss, b.cvss) {
                (Some(a_score), Some(b_score)) => b_score.partial_cmp(&a_score)
...
mod tests {
    ...
    fn test_parse_exploit_line() {
        ...
        assert_eq!(result.url, "https://www.exploit-db.com/exploit | Description");
    }
}